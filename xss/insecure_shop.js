// insecure_shop_final.js (Pro-Style + Login/CSRF Flaw)
const express = require('express');
const session = require('express-session');
const cookieParser = require('cookie-parser');
const csrf = require('csurf'); // <-- For CSRF protection
const he = require('he'); // <-- We have this, but won't use it on the flaw
const app = express();
...
app.use(session({
    secret: 'a-very-secret-key-for-our-shop',
    resave: false,
...
// --- Main E-Shop Page (Now requires login) ---
app.get('/', requireLogin, (req, res) => {
    let reviewsHtml = '';

    // --- THE VULNERABILITY (Unchanged) ---
    // We are still injecting raw text.
    ...
    <body class="pb-5">
        <nav class="navbar navbar-expand-lg navbar-light sticky-top">
          <div class="container">
            <a class="navbar-brand fw-bold" href="#"><i class="bi bi-shop me-2"></i>E-Shop Pro</a>
...
// The "Buy Now" button is secure from CSRF
...
// The "Review" post is secure from CSRF...
// ...but it insecurely STORES the review text.
app.post('/review', requireLogin, (req, res) => {
    const { user, text } = req.body;
    if (user && text) {
        // We encode the USER, but not the TEXT. This is our "flaw".
